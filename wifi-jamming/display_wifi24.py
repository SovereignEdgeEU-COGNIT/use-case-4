...
channelcenter = [2412e6, 2417e6, 2422e6, 2427e6, 2432e6, 2437e6, 2442e6, 2447e6, 2452e6, 2457e6, 2462e6, 2467e6, 2472e6]
...
for x in sorted(y):
	for ch in channels.keys():
		if channels[ch][0][0] < x <= channels[ch][0][1]:
			#print(ch)
			if not ch in chsum.keys():
				chsum[ch] = y[x]
			else:
				chsum[ch] = (
				chsum[ch] + y[x])#/2
plt.plot(channelcenter, chsum.values(), 'bo')
plt.plot(channelcenter, chsum.values(), 'b-')
#for ch in channels.keys():
